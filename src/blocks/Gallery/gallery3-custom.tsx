...
const GalleryItem = ({
  ...
  item: NonNullable<GalleryBlock['elements']>[number]
  ...
  return (
    <CarouselItem key={item.id} className="max-w-[380px] p-0 sm:max-w-[305px] lg:max-w-[340px]">
      <CMSLink
      ...
      >
        ...
        <div className="mt-6">
          {item.label && <Badge className="rounded-md font-normal">{item.label}</Badge>}
        </div>
...
const Gallery3Custom: React.FC<GalleryBlock & { publicContext: PublicContextProps }> = ({
  ...
  return (
    <section className={cn('mx-auto lg:max-w-[1280px]', getSpacings(spacings))} ref={ref}>
      <div>
