...
const Feature114: React.FC<
  FeatureBlock & { publicContext: PublicContextProps }
> = ({ richText, USPs, links, publicContext, images, metrics }) => {
  ...
  const features = USPs?.length
    ? USPs.map((usp, index) => {
        ...
        const description =
          usp.richText?.root?.children?.[0]?.children?.[0]?.text || "";

        // Use custom icon if provided, or try to find a default icon, or use the first default icon
        let iconElement: React.ReactNode;
        if (usp.uspIcon) {
          iconElement = (
            <Icon icon={usp.uspIcon} className="h-auto w-8 md:w-12" />
          );
        } else {
