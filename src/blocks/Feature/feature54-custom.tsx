...
const ActiveTabContent: React.FC<{
  ...
  const { firstNode: firstNodeDescription, rest: restDescription } = splitRichText(richtext, {
    splitOn: ['h2', 'h3', 'h4'],
    takeFirst: true,
  })

  return (
    <div key={feature.id} className="mx-auto mt-12 max-w-[426px] px-6 md:max-w-[564px] md:px-8">
      {firstNodeDescription && fillFromDescription && extractPlainText(firstNodeDescription) && (
        <RichText
      ...
      {restDescription && fillFromDescription && extractPlainText(restDescription) && (
        <RichText
          publicContext={publicContext}
      ...
      {feature.links && feature.links.length > 0 && (
        <div
          className={cn(
            'mt-12 flex flex-col justify-center gap-4 md:flex-row md:items-center md:gap-6',
            feature.links?.length === 1 && 'items-center',
          )}
        >
          {feature.links?.map((link) => {
            return <CMSLink key={link.id} {...link.link} publicContext={publicContext} />
          })}
        </div>
      )}
    </div>
