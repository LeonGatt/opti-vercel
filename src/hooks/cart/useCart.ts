...
export const useCart = (storeName = 'optitrack') => {
  ...
  const { storedCart, storedCartId, setStoredCart, removeStoredCart } = useCartStorage()
  ...
  const addItemMutation = useMutation({
    mutationFn: async ({ itemId, quantity = 1 }: { itemId: number; quantity?: number }) => {
      ...
      const response = await fetch(`${baseUrl}/${cartId}/${itemId}/${quantity}`, {
        method: 'POST',
  ...
  const cartIsEmpty = useMemo(() => !cart || !cart.items || cart.items?.length === 0, [cart])
  return {
    cart: cart || null,
    cartIsEmpty,
    isLoading,
    error,
    addItem: (itemId: number, quantity = 1) => addItemMutation.mutateAsync({ itemId, quantity }),
