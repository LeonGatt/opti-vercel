...
const BackupDashboard: React.FC<BackupDashboardProps> = async ({
  user,
  i18n,
  searchParams,
}) => {
  ...
  if (!process.env.DATABASE_URI || !process.env.BLOB_READ_WRITE_TOKEN) {
    return;
  }
    ...
    <div className="backup-dashboard">
      ...
      <Collapsible initCollapsed={true}>
        ...
        {sortedBlobs.map((blob) => {
          const { type, dbName, hostname, fileType } = transformBlobName(
            blob.pathname,
          );
          ...
          return (
            <div key={blob.pathname} className="backup-item">
              <p>
                ...
                <span className={isCurrentDb ? "" : "red-text"}>
                  DB: {dbName || "Unknown"},{" "}
                </span>
              ...
              <div className="right">
                ...
                >
                  <div>
                    Restoring this Backup will may override some of your newly
                    created data. Are you sure you want to continue?
                  </div>
                  <Button
                    className="btn-red"
                    onClick={async () => {
        ...
        <div className="make-backup-container">
          ...
          <span className="text">
            Manual backups will not get automatically deleted
          </span>
        </div>
      </Collapsible>
    </div>
