...
export const CMSLink: React.FC<CMSLinkType> = (props) => {
  ...
  if (
    ...
    const normalizedRemainingPath =
      reference.value.slug === "home" ? "" : remainingPath;
  ...
  const content = (
    <>
      {iconBefore && (
        <Icon className={cn("mr-2 h-6", iconClassName)} icon={iconBefore} />
      )}
      {label && label}
      {children && children}
      {iconAfter && (
        <Icon className={cn("ml-2 h-6", iconClassName)} icon={iconAfter} />
      )}
    </>
  );
