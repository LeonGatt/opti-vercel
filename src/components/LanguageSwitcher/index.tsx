              ...
              {locales.map((locale) => {
                ...
                if (currentLocale === locale) {
                  return (
                    <span
                      key={locale}
                      className="w-[85px] mb-1 p-2 text-accent-foreground bg-card rounded-md flex items-center justify-start"
                    >
                  ...
                  return (
                    <Link
                      ...
                      href={href}
                      lang={locale}
                      className="w-[85px] mb-1 p-2 hover:text-accent-foreground hover:bg-card rounded-md flex items-center justify-start"
                    >
                      {localeLabels[locale]}
                    </Link>
                  )
        ...
        {locales.map((locale) => {
          ...
          if (currentLocale === locale) {
            return (
              <span
                key={locale}
                className="text-sm font-semibold flex select-none gap-4 rounded-md p-3 leading-none outline-hidden transition-colors hover:bg-accent hover:text-accent-foreground focus:bg-accent focus:text-accent-foreground"
              >
                {localeLabels[locale]} <Check className="w-4 h-4 ml-2" />
              </span>
            )
