...
export const ThemeColorHelper: React.FC<{}> = () => {
    ...
    const mouseover = (event: MouseEvent) => {
      ...
      const c = classNames.filter((cn) =>
        colors.some((color) => {
          ...
          if (
            cn.match(/shadow-(lg|md|sm|xs|xl|2xl|3xl|4xl|5xl|6xl|7xl|8xl|9xl)/)
          ) {
            return false;
          }
          if (
            cn.match(/text-(lg|md|sm|xs|xl|2xl|3xl|4xl|5xl|6xl|7xl|8xl|9xl)/)
          ) {
            return false;
          }
  ...
  useEffect(() => {
    ...
    let radiusValue = themeConfigElement?.innerHTML?.match(
      /--radius: ([0-9\.]*)rem/,
    )?.[1];
    if (radiusValue) {
