...
const generateTitle: GenerateTitle<Post | Page> = ({ doc }) => {
  return doc?.title
    ? `${doc.title} | Payblocks Website Template`
    : "Payblocks Website Template";
};
...
export default buildConfig({
  admin: {
    components: {
      ...
      afterLogin: googleAuthActive
        ? ["@/components/AdminDashboard/LoginButton"]
        : [],
  ...
  globals: [Header, Footer, PageConfig],
  plugins: [
    redirectsPlugin({
    ...
    OAuth2Plugin({
      ...
      getUserInfo: async (accessToken: string) => {
        ...
        if (
          allowedDomains &&
          !allowedDomains.includes(user.email.split("@")?.[1])
        ) {
